export default class GridUtils {
    ...
    snapToGrid(sprite) {
        ...
        const clampedX = Phaser.Math.Clamp(startX, 0, this.scene.gridLogicalWidth - w);
        const clampedY = Phaser.Math.Clamp(startY, 0, this.scene.gridLogicalHeight - h);
    ...
    getSpriteFootprint(sprite) {
        // Pega os dados do item diretamente no sprite
        const data = sprite.itemData;

        if (!data) return { w: 1, h: 1, wL: 1, hL: 1 };

        const logicFactor = this.scene.gridSize / this.scene.gridLogicalSize;

        return {
            w: data.wV ?? 1,           // tamanho visual
            h: data.hV ?? 1,
            wL: data.wL ?? Math.max(1, Math.round((data.wV ?? 1) * logicFactor)),  // tamanho lógico
            hL: data.hL ?? Math.max(1, Math.round((data.hV ?? 1) * logicFactor))
        };
    }
    ...
    // --- Marca sprite no grid lógico ---
    markOccupied(sprite, startX, startY, w, h) {
        const maxX = this.scene.gridMap.length;
        const maxY = this.scene.gridMap[0].length;
    ...
    clearOccupied(sprite) {
        for (let x = 0; x < this.scene.gridMap.length; x++) {
            for (let y = 0; y < this.scene.gridMap[x].length; y++) {
    ...
    // --- Verifica ocupação no grid lógico ---
    checkOccupiedGrid(startLX, startLY, wL = 1, hL = 1, ignoreSprite = null) {
        const map = this.scene.gridMap;

        for (let x = startLX; x < startLX + wL; x++) {
            for (let y = startLY; y < startLY + hL; y++) {
                // Verifica se estamos dentro dos limites do grid lógico
                if (x < 0 || y < 0 || x >= map.length || y >= map[0].length) {
                    return true; // Fora do limite = considerado ocupado
                }

                // Se já há algo no tile que não seja o próprio sprite
                const cell = map[x][y];
                if (cell && cell !== ignoreSprite) {
                    return true;
                }
            }
        }
    ...
    drawFootprints() {
        ...
        for (let sprite of this.scene.sprites) {
            ...
            for (let i = 0; i < wL; i++) {
                for (let j = 0; j < hL; j++) {
                    const p1 = this.isoToScreen((startLX + i) / logicFactor, (startLY + j) / logicFactor, this.scene.gridSize, this.scene.offsetX, this.scene.offsetY);
                    const p2 = this.isoToScreen((startLX + i + 1) / logicFactor, (startLY + j) / logicFactor, this.scene.gridSize, this.scene.offsetX, this.scene.offsetY);
                    const p3 = this.isoToScreen((startLX + i + 1) / logicFactor, (startLY + j + 1) / logicFactor, this.scene.gridSize, this.scene.offsetX, this.scene.offsetY);
                    const p4 = this.isoToScreen((startLX + i) / logicFactor, (startLY + j + 1) / logicFactor, this.scene.gridSize, this.scene.offsetX, this.scene.offsetY);
